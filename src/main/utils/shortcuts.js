...
function registerShortcuts() {
  ...
  console.log(
    "Global shortcuts registered: Ctrl+Alt+C, Ctrl+Alt+Right, Ctrl+Alt+A, Ctrl+Alt+F",
  );
}
...
async function triggerFieldFillMode() {
  await runShortcutFlow({
    ...
    buildResponse: async (captureResult) =>
      smartWriterService.autoFill(captureResult),
  });
}
...
async function triggerFormFillMode() {
  ...
  if (!cancelRegistered) {
    console.warn(
      "Failed to register Escape shortcut for form automation cancellation",
    );
  }
  ...
  try {
    ...
    const initialState = await formFillerService.inspectFormState(
      captureResult.base64,
      { minimumFields: 2 },
    );
    if (!initialState.isForm) {
      hudManager.show(
        `${WAND_ICON} No form detected. Open a form and focus the first input.`,
      );
      await sleep(2200);
      hudManager.reset();
      return;
    ...
    await hudManager.startCountdown(
      getConfiguredCountdown(),
      "Focus first input. Starting in",
      false,
    );
    ...
    const result = await formAutomationService.run({
      initialState,
      onStatus: (message) =>
        hudManager.show(`${WAND_ICON} ${message} (Esc to stop)`),
    });
    ...
    const wasCancelled =
      error?.name === "FormAutomationCancelledError" ||
...
function formatFormAutomationResult(result) {
  ...
  if (result.selectedChoiceCount > 0) {
    detailParts.push(
      `${result.selectedChoiceCount} choice${result.selectedChoiceCount === 1 ? "" : "s"}`,
    );
  }
